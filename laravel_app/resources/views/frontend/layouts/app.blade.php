<!DOCTYPE html>
<html lang="en">

<!-- Mirrored from html.awaikenthemes.com/lenity/ by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 19 Feb 2026 09:31:14 GMT -->
<head>
    <!-- Meta -->
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1">
    <meta name="description" content="">
    <meta name="keywords" content="">
    <meta name="author" content="Awaiken">
     @php
        $headerFooterData = is_array($headerFooterData ?? null) ? $headerFooterData : [];
        $websiteName = $headerFooterData['website_name'] ?? config('app.name');
        $faviconRaw = $headerFooterData['favicon'] ?? null;
        $faviconUrl = filled($faviconRaw)
            ? asset('storage/' . ltrim((string) $faviconRaw, '/'))
            : asset('images/favicon.png');
    @endphp
    <!-- Page Title -->
    <title>{{ $websiteName }}</title>
    <!-- Favicon Icon -->
    <link rel="shortcut icon" type="image/x-icon" href="{{ $faviconUrl }}">
    <!-- Google Fonts Css-->
    <link rel="preconnect" href="https://fonts.googleapis.com/">
    <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&amp;family=Onest:wght@100..900&amp;display=swap" rel="stylesheet">
    <!-- Bootstrap Css -->
    <link href="{{asset('frontend/assets/css/bootstrap.min.css')}}" rel="stylesheet" media="screen">
    <!-- SlickNav Css -->
    <link href="{{asset('frontend/assets/css/slicknav.min.css')}}" rel="stylesheet">
    <!-- Swiper Css -->
    <link rel="stylesheet" href="{{asset('frontend/assets/css/swiper-bundle.min.css')}}">
    <!-- Font Awesome Icon Css-->
    <link href="{{asset('frontend/assets/css/all.min.css')}}" rel="stylesheet" media="screen">
    <!-- Animated Css -->
    <link href="{{asset('frontend/assets/css/animate.css')}}" rel="stylesheet">
    <!-- Magnific Popup Core Css File -->
    <link rel="stylesheet" href="{{asset('frontend/assets/css/magnific-popup.css')}}">
    <!-- Mouse Cursor Css File -->
    <link rel="stylesheet" href="{{asset('frontend/assets/css/mousecursor.css')}}">
    <!-- Main Custom Css -->
    <link href="{{asset('frontend/assets/css/custom.css')}}" rel="stylesheet" media="screen">
    @stack('css')
</head>
<body>

<!-- Preloader Start -->
{{--<div class="preloader">--}}
{{--    <div class="loading-container">--}}
{{--        <div class="loading"></div>--}}
{{--        <div id="loading-icon"><img src="images/loader.svg" alt=""></div>--}}
{{--    </div>--}}
{{--</div>--}}
<!-- Preloader End -->
@include('frontend.includes.header')

@yield('content')

@include('frontend.includes.footer')

<!-- Jquery Library File -->
<script src="{{asset('frontend/assets/js/jquery-3.7.1.min.js')}}"></script>
<!-- Bootstrap js file -->
<script src="{{asset('frontend/assets/js/bootstrap.min.js')}}"></script>
<!-- Validator js file -->
<script src="{{asset('frontend/assets/js/validator.min.js')}}"></script>
<!-- SlickNav js file -->
<script src="{{asset('frontend/assets/js/jquery.slicknav.js')}}"></script>
<!-- Swiper js file -->
<script src="{{asset('frontend/assets/js/swiper-bundle.min.js')}}"></script>
<!-- Counter js file -->
<script src="{{asset('frontend/assets/js/jquery.waypoints.min.js')}}"></script>
<script src="{{asset('frontend/assets/js/jquery.counterup.min.js')}}"></script>
<!-- Isotop js file -->
<script src="{{asset('frontend/assets/js/isotope.min.js')}}"></script>
<!-- Magnific js file -->
<script src="{{asset('frontend/assets/js/jquery.magnific-popup.min.js')}}"></script>
<!-- SmoothScroll -->
<script src="{{asset('frontend/assets/js/SmoothScroll.js')}}"></script>
<!-- Parallax js -->
<script src="{{asset('frontend/assets/js/parallaxie.js')}}"></script>
<!-- MagicCursor js file -->
<script src="{{asset('frontend/assets/js/gsap.min.js')}}"></script>
<script src="{{asset('frontend/assets/js/magiccursor.js')}}"></script>
<!-- Text Effect js file -->
<script src="{{asset('frontend/assets/js/SplitText.js')}}"></script>
<script src="{{asset('frontend/assets/js/ScrollTrigger.min.js')}}"></script>
<!-- YTPlayer js File -->
<script src="{{asset('frontend/assets/js/jquery.mb.YTPlayer.min.js')}}"></script>
<!-- Wow js file -->
<script src="{{asset('frontend/assets/js/wow.min.js')}}"></script>
<!-- Main Custom js file -->
<script src="{{asset('frontend/assets/js/function.js')}}"></script>
@stack('js')
</body>

<!-- Mirrored from html.awaikenthemes.com/lenity/ by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 19 Feb 2026 09:31:14 GMT -->
</html>
